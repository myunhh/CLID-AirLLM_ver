...
// This is intentionally a sparse graph, rather than an ordered list: a member may be
// reactivated from reported or blocked, but never revived once archived.
export const MEMBER_TRANSITIONS = {
	pending: ["active", "blocked", "archived"],
	active: ["reported", "blocked", "archived"],
	reported: ["active", "archived"],
	blocked: ["active", "archived"],
	archived: [],
};
export const TEAM_TRANSITIONS = { active: ["archived"], archived: [] };
...
function assertMemberTransition(member, status) {
	const allowed = MEMBER_TRANSITIONS[member.status];
	if (!allowed?.includes(status)) {
		throw new Error(`ILLEGAL_MEMBER_TRANSITION: ${member.status} -> ${status}`);
	}
}

function assertTeamTransition(team, status) {
	const allowed = TEAM_TRANSITIONS[team.status];
	if (!allowed?.includes(status)) {
		throw new Error(`ILLEGAL_TEAM_TRANSITION: ${team.status} -> ${status}`);
	}
}
...
export function bindThread(team, { id, threadId, cwd = null, worktreePath = null }) {
	...
	assertMemberTransition(m, "active");
...
export function setMemberStatus(team, { id, status, note = "" }) {
	...
	assertMemberTransition(member, status);
	member.status = status;
	return touch(team, "set-status", `member ${id} -> ${status}${note ? `: ${note}` : ""}`);
}

export function setTeamStatus(team, { status, note = "" }) {
	if (!Object.hasOwn(TEAM_TRANSITIONS, status)) throw new Error(`invalid team status "${status}" - use one of: ${Object.keys(TEAM_TRANSITIONS).join(", ")}`);
	assertTeamTransition(team, status);
	team.status = status;
	if (status === "archived") team.archivedAt = isoNow();
	return touch(team, "set-team-status", `team -> ${status}${note ? `: ${note}` : ""}`);
}
...
export function archive(team, { id = null, note = "" } = {}) {
	if (id) {
		const member = memberById(team, id);
		assertMemberTransition(member, "archived");
	...
	assertTeamTransition(team, "archived");
